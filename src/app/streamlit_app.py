...
def main():
    ...
    with tab1:
        ...
        if os.path.exists(config.CHATS_DIR):
            chats = [d for d in os.listdir(config.CHATS_DIR) if os.path.isdir(os.path.join(config.CHATS_DIR, d))]
            existing_chats += sorted(chats)
    ...
    with tab2:
        ...
        if len(existing_chats) > 1:
            ...
            # Reset profile if selection changes
            if st.session_state.profile_contact and st.session_state.profile_contact != contact_to_profile:
                st.session_state.current_profile = None
                st.session_state.profile_contact = None
            ...
            if st.session_state.current_profile and st.session_state.profile_contact:
                ...
                st.download_button(
                    label="Download Profile (Markdown)",

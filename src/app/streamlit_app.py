...
def main():
    ...
    with sidebar.expander("Instagram Login", expanded=not st.session_state.logged_in):
        ...
        if st.button("Login", help="Securely authenticate with your Instagram account"):
    ...
    if st.session_state.logged_in:
        ...
        if sidebar.button("Start Background Sync", help="Begin automatic periodic syncing of new messages"):
            st.session_state.sync_engine.start()
            st.sidebar.info("Background sync running...")

        if sidebar.button("Stop Background Sync", help="Halt the automatic syncing process"):
    ...
    if sidebar.button("Import & Index Data", help="Process the unzipped Instagram data and make it searchable"):
    ...
    with tab1:
        ...
        if st.button("Search AI", type="primary", help="Use AI to find answers within your chat history"):
            if query:
    ...
    with tab2:
        ...
        if len(existing_chats) > 1:
            ...
            if st.button("Generate Psychological Profile", help="Analyze communication patterns to generate a personality profile"):
            ...
            if 'current_profile' in st.session_state and st.session_state.profile_contact == contact_to_profile:
                ...
                st.download_button(
                    ...
                    mime="text/markdown",
                    help="Export this profile to a file"
                )
        else:

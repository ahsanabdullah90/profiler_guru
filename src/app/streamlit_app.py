...
def main():
    ...
    with sidebar.expander("Instagram Login", expanded=not st.session_state.logged_in):
        ...
        if st.button("Login", help="Authenticate with your Instagram credentials to start syncing messages."):
    ...
    if st.session_state.logged_in:
        ...
        if sidebar.button("Start Background Sync", help="Periodically fetch new messages from Instagram in the background."):
            st.session_state.sync_engine.start()
            st.sidebar.info("Background sync running...")

        if sidebar.button("Stop Background Sync", help="Stop the background sync process."):
    ...
    if sidebar.button("Import & Index Data", help="Process and index historical Instagram data from the specified path."):
    ...
    with tab1:
        ...
        if st.button("Search AI", type="primary", help="Query the AI about your chat history based on the provided prompt."):
            if query:
    ...
    with tab2:
        ...
        if len(existing_chats) > 1:
            ...
            if st.button("Generate Psychological Profile", help="Analyze communication patterns to generate a psychological assessment."):
                with st.spinner(f"Analyzing communication patterns for {contact_to_profile}..."):
                    ...
                    st.session_state.current_profile_name = contact_to_profile

            if st.session_state.current_profile:
                st.write(f"### Profile for {st.session_state.current_profile_name}")
                st.markdown(st.session_state.current_profile)
                st.download_button(
                    label="Download Report",
                    data=st.session_state.current_profile,
                    file_name=f"profile_{st.session_state.current_profile_name}.md",
                    mime="text/markdown",
                    help="Download this psychological assessment as a Markdown file."
                )
        else:
